...
function removeFirstOccurrences(str, value) {
  ...
  const newStr = str.slice(index, index + value.length);
...
function isPalindrome(str) {
  const newStr = str.replace(/[?,!]/g, '').toLowerCase().split(' ').join('');
...
function findLongestWord(sentence) {
  const dataArr = sentence.split(' ');
  let max = dataArr[0];
  for (let i = 1; i < dataArr.length; ) {
    if (dataArr[i].length > max.length) max = dataArr[i];
    i += 1;
  }
  return max;
}
...
function extractNameFromTemplate(value) {
  return `${value.split(' ')[1]} ${value.replace(/[!]/g, '').split(' ')[2]}`;
}
...
function encodeToRot13(str) {
  ...
  const codeData = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

  return str.replace(/[a-z]/gi, (symb) => codeData[data.indexOf(symb)]);
}
